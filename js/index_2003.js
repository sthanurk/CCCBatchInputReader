...
(function() {
	...
	for (i = 1; i <= k; i++) {
		...
		if (x.id == "c8" || x.id == "c28" || x.id == "c29" || x.id == "c36" || x.id == "c37" || x.id == "c42" || x.id == "c45" || x.id == "c46" || x.id == "c47" || x.id == "c48" || x.id == "c49" || x.id == "c50" || x.id == "c51" || x.id == "c52") {
			x.style.width = "8px";
		}
	}
...
function newrecord(event_type) {
	...
	for (var i = 0; i < col; i++) {
		...
		var previous_signer_consumer_name = x.rows[len].cells[9].getElementsByTagName("input")[0].value;
		var previous_signer_business_name = x.rows[len].cells[23].getElementsByTagName("input")[0].value;
		if (value % col == 13 &&  previous_signer_business_name == "                                        ") {
			var previous_ssn = x.rows[len].cells[12].getElementsByTagName("input")[0].value;
			new_cell[i].value = parseInt(previous_ssn, 10) + 1;
		}
		if (value % col == 26 &&  previous_signer_consumer_name == "            ") {
			var previous_ein = x.rows[len].cells[25].getElementsByTagName("input")[0].value;
...
function generate_accnos() {
	...
	var signer_consumer_name = batch_text.substr(70, 12);
	var signer_business_name = batch_text.substr(209, 40);
	if (signer_business_name == "                                        ") {
		var start_ssn = batch_text.substr(93, 9);
	}
	if (signer_consumer_name == "            ") {
		var start_ssn = batch_text.substr(250, 9);
	}
	var count = document.getElementById('ac_count').value;
	...
	for (g = 1; g < count; g++) {
		...
		if (signer_consumer_name == "            ") {
			text = text.replace(text.substr(250, 9), new_ssn);
		}
...
function saveTextAsFile(clicked_button) {
	...
	var fileNameToSaveAs = "CCC2017_2003R";
...
function Writedata() {
	...
	*/
	...
	var newBN = article[24].value; //--For Business-Name
	var newSSN = parseInt(article[13].value, 10); //Taking SSN value from Array
	var federalTaxID = parseInt(article[26].value, 10); //Federal Tax ID for Business record
	while (count > 0) {
		var totalstr;
		totalstr = '';
		for (i = 1; i <= 58; i++) {
...
function Maxlength_Verify() {
	...
	if (error_count == numberOfLineBreaks && batch_text!="") {
		alert("Please input the records in 2003R Format.");
	}	
