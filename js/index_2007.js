...
(function() {
	...
	for (i = 1; i <= k; i++) {
		...
		if (x.id == "c8" || x.id == "c24" || x.id == "c29" || x.id == "c41" || x.id == "c42" || x.id == "c47" || x.id == "c61" || x.id == "c78" || x.id == "c79" || x.id == "c80" || x.id == "c81" || x.id == "c82" || x.id == "c83") {
			x.style.width = "8px";
		}
	}
...
function newrecord(event_type) {
	...
	for (var i = 0; i < col; i++) {
		...
		var previous_signer_business_name = x.rows[len].cells[24].getElementsByTagName("input")[0].value;
		...
		if (value % col == 27 &&  previous_signer_consumer_name == "            ") {
...
function generate_accnos() {
	...
	var signer_business_name = batch_text.substr(212, 40);
...
function saveTextAsFile(clicked_button) {
	...
	var fileNameToSaveAs = "CCC2017_2007R";
...
function Writedata() {
	...
	*/
	...
	var Corporation_Flag = article[26].value; //Y or N or Blank
	var newBN = article[25].value; //Business name
	var newFN = article[10].value; //Account Holder's Last Name
	var federalTaxID = article[27].value; //federalTaxID
	console.log(newBN);
	while (count > 0) {
		var totalstr;
		totalstr = '';
...
function Maxlength_Verify() {
	...
	if (error_count == numberOfLineBreaks && batch_text!="") {
		alert("Please input the records in 2007R Format.");
	}	
